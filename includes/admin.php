...
add_action( 'wp_ajax_cm_reset_theme_defaults', 'cm_ajax_reset_theme_defaults' );
function cm_ajax_reset_theme_defaults() {
    check_ajax_referer( 'cm_save_settings', 'nonce' );
    if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Geen toegang' );
...
function cm_ajax_reset_license() {
    check_ajax_referer( 'cm_save_settings', 'nonce' );
    if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Geen toegang' );
...
function cm_ajax_export_settings() {
    ...
    $settings = get_option( 'cm_settings', cm_default_settings() );
    // Licentiegegevens nooit exporteren
...
/**
 * Rendert alle kleurvelden voor een thema ('light' of 'dark').
 * Light gebruikt de normale kleursleutels, Dark gebruikt dm_* sleutels.
 */
function cm_render_theme_fields( $theme, $s ) {
    $p = ( $theme === 'dark' ) ? 'dm_' : 'color_';  // prefix: light=color_, dark=dm_
    // Radius en opacity hebben geen color_ prefix bij light
    $rp_key  = ( $theme === 'dark' ) ? 'dm_radius_popup'    : 'radius_popup';
    $rb_key  = ( $theme === 'dark' ) ? 'dm_radius_btn'      : 'radius_btn';
    $op_key  = ( $theme === 'dark' ) ? 'dm_overlay_opacity' : 'overlay_opacity';
    // Helper: kleurpicker met prefix
    $f  = function( $key ) use ( $p, $s ) { cm_color_field( $p . $key, $s ); };
    $fo = function( $key ) use ( $p, $s ) { cm_color_field_optional( $p . $key, $s ); };

    ob_start();
    ?>
    <div class="cm-group cm-accordion" style="border-top:none;border-radius:0 0 4px 4px">
        <h3 class="cm-group-title cm-accordion-head" data-acc="tf-float-<?php echo $theme; ?>">
            <span class="cm-acc-icon">+</span> Zweefknop kleuren
        </h3>
        <div class="cm-accordion-body" id="cm-acc-tf-float-<?php echo $theme; ?>" style="display:none">
        <table class="form-table cm-form-table"><tbody>
        <tr><td colspan="2"><h4 class="cm-btn-sub-head">Kleuren icoontje</h4></td></tr>
        <tr><th><label>Achtergrond</label></th><td><?php $f('float_icon_bg'); ?></td></tr>
        <tr><th><label>Icoontje kleur</label></th><td><?php $f('float_icon_color'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $f('float_icon_hover_bg'); ?></td></tr>
        <tr><th><label>Icoontje kleur hover</label></th><td><?php $f('float_icon_hover_color'); ?></td></tr>
        <tr><td colspan="2"><h4 class="cm-btn-sub-head">Kleuren tekstknop</h4></td></tr>
        <tr><th><label>Achtergrond</label></th><td><?php $f('float_text_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur</label></th><td><?php $f('float_text_color'); ?></td></tr>
        <tr><th><label>Randkleur</label></th><td><?php $f('float_text_border'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $f('float_text_hover_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur hover</label></th><td><?php $f('float_text_hover_color'); ?></td></tr>
        </tbody></table>
        </div>
    </div>

    <div class="cm-group cm-accordion" style="border-radius:0">
        <h3 class="cm-group-title cm-accordion-head" data-acc="tf-popup-<?php echo $theme; ?>">
            <span class="cm-acc-icon">+</span> Popup kleuren
            <span class="cm-acc-sub">Achtergrond, titels, tekst en links</span>
        </h3>
        <div class="cm-accordion-body" id="cm-acc-tf-popup-<?php echo $theme; ?>" style="display:none">
        <table class="form-table cm-form-table"><tbody>
        <tr><th><label>Achtergrond</label></th><td><?php $f('popup_bg'); ?></td></tr>
        <tr><th><label>Kleur titels</label></th><td><?php $f('title'); // light: color_title, dark: dm_title ?></td></tr>
        <tr><th><label>Kleur bodytekst</label></th><td><?php $f('body'); ?></td></tr>
        <tr><th><label>Kleur links</label></th><td><?php $f('link'); ?></td></tr>
        <tr><th><label>Border radius popup</label></th><td><?php cm_radius_field( $rp_key, $s, 0, 60 ); ?></td></tr>
        <tr><th><label>Overlay donkerte</label></th><td><?php cm_range_field( $op_key, $s, 0, 90, '%' ); ?><p class="description">Donkerte van de achtergrond achter de popup.</p></td></tr>
        </tbody></table>
        </div>
    </div>

    <div class="cm-group cm-accordion" style="border-radius:0">
        <h3 class="cm-group-title cm-accordion-head" data-acc="tf-buttons-<?php echo $theme; ?>">
            <span class="cm-acc-icon">+</span> Buttons
            <span class="cm-acc-sub">Akkoord, Weigeren, Voorkeuren, Alle cookies toestaan</span>
        </h3>
        <div class="cm-accordion-body" id="cm-acc-tf-buttons-<?php echo $theme; ?>" style="display:none">
        <table class="form-table cm-form-table"><tbody>
        <tr><td colspan="2"><h4 class="cm-sub-head cm-btn-sub-head">Akkoord button</h4></td></tr>
        <tr><th><label>Achtergrond</label></th><td><?php $f('accept_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur</label></th><td><?php $f('accept_text'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $f('accept_hover_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur hover</label></th><td><?php $f('accept_hover_text'); ?></td></tr>
        <tr><th><label>Randkleur (optioneel)</label></th><td><?php $fo('accept_border'); ?></td></tr>
        <tr><td colspan="2"><h4 class="cm-sub-head cm-btn-sub-head">Weigeren button</h4></td></tr>
        <tr><th><label>Achtergrond</label></th><td><?php $f('reject_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur</label></th><td><?php $f('reject_text'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $f('reject_hover_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur hover</label></th><td><?php $f('reject_hover_text'); ?></td></tr>
        <tr><th><label>Randkleur (optioneel)</label></th><td><?php $fo('reject_border'); ?></td></tr>
        <tr><td colspan="2"><h4 class="cm-sub-head cm-btn-sub-head">Cookie voorkeuren button</h4></td></tr>
        <tr><th><label>Randkleur</label></th><td><?php $f('prefs_border'); ?></td></tr>
        <tr><th><label>Tekstkleur</label></th><td><?php $f('prefs_text'); ?></td></tr>
        <tr><th><label>Randkleur hover</label></th><td><?php $f('prefs_hover_border'); ?></td></tr>
        <tr><th><label>Tekstkleur hover</label></th><td><?php $f('prefs_hover_text'); ?></td></tr>
        ...
        <tr><th><label>Achtergrond</label></th><td><?php $f('allowall_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur</label></th><td><?php $f('allowall_text'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $f('allowall_hover_bg'); ?></td></tr>
        <tr><th><label>Tekstkleur hover</label></th><td><?php $f('allowall_hover_text'); ?></td></tr>
        <tr><th><label>Randkleur (optioneel)</label></th><td><?php $fo('allowall_border'); ?></td></tr>
        ...
        <tr><th><label>Randkleur</label></th><td><?php $f('outline_border'); ?></td></tr>
        <tr><th><label>Tekstkleur</label></th><td><?php $f('outline_text'); ?></td></tr>
        <tr><th><label>Randkleur hover</label></th><td><?php $f('outline_hover_border'); ?></td></tr>
        <tr><th><label>Tekstkleur hover</label></th><td><?php $f('outline_hover_text'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $fo('outline_hover_bg'); ?></td></tr>
        <tr><td colspan="2"><h4 class="cm-sub-head">Border radius buttons</h4></td></tr>
        <tr><th><label>Afronding</label></th><td><?php cm_radius_field( $rb_key, $s, 0, 60 ); ?><p class="description">Geldt voor alle buttons.</p></td></tr>
        </tbody></table>
        </div>
    </div>

    <div class="cm-group cm-accordion" style="border-radius:0">
        <h3 class="cm-group-title cm-accordion-head" data-acc="tf-toggles-<?php echo $theme; ?>">
            <span class="cm-acc-icon">+</span> Sluit-knop, Toggles &amp; Cookielijst
        </h3>
        <div class="cm-accordion-body" id="cm-acc-tf-toggles-<?php echo $theme; ?>" style="display:none">
        <table class="form-table cm-form-table"><tbody>
        <tr><th><label>Achtergrond kruisje</label></th><td><?php $f('close_bg'); ?></td></tr>
        <tr><th><label>Achtergrond hover kruisje</label></th><td><?php $f('close_hover_bg'); ?></td></tr>
        <tr><th><label>Kleur kruisje</label></th><td><?php $f('close_icon'); ?></td></tr>
        ...
        <tr><th><label>Achtergrond (open)</label></th><td><?php $f('expand_open_bg'); ?><p class="description">Achtergrond wanneer de categorie is uitgeklapt.</p></td></tr>
        <tr><th><label>Icoon kleur (open)</label></th><td><?php $f('expand_open_icon'); ?></td></tr>
        <tr><td colspan="2"><h4 class="cm-sub-head cm-btn-sub-head">Cookielijst in popup</h4></td></tr>
        <tr><th><label>Categorie hover achtergrond</label></th><td><?php $f('cat_header_hover'); ?></td></tr>
        <tr><th><label>Categorie beschrijving kleur</label></th><td><?php $f('cat_desc'); ?></td></tr>
        <tr><th><label>Detail tekst kleur</label></th><td><?php $f('cat_detail'); ?></td></tr>
        <tr><th><label>Cookie naam kleur</label></th><td><?php $f('cookie_name'); ?></td></tr>
    ...
    <div class="cm-group cm-accordion" style="border-radius:0 0 4px 4px">
        <h3 class="cm-group-title cm-accordion-head" data-acc="tf-embed-<?php echo $theme; ?>">
        ...
        <div class="cm-accordion-body" id="cm-acc-tf-embed-<?php echo $theme; ?>" style="display:none">
        <table class="form-table cm-form-table"><tbody>
        <tr><th><label>Achtergrond</label></th><td><?php $f('embed_bg'); ?></td></tr>
        <tr><th><label>Titel kleur</label></th><td><?php $f('embed_title'); ?></td></tr>
        <tr><th><label>Tekst kleur</label></th><td><?php $f('embed_body'); ?></td></tr>
        <tr><td colspan="2"><h4 class="cm-btn-sub-head">Knop &ldquo;Inhoud laden&rdquo;</h4></td></tr>
        <tr><th><label>Achtergrond</label></th><td><?php $f('embed_btn_bg'); ?></td></tr>
        <tr><th><label>Tekst</label></th><td><?php $f('embed_btn_text'); ?></td></tr>
        <tr><th><label>Achtergrond hover</label></th><td><?php $f('embed_btn_hover_bg'); ?></td></tr>
        <tr><th><label>Tekst hover</label></th><td><?php $f('embed_btn_hover_text'); ?></td></tr>
        </tbody></table>
        </div>
    </div>
    <?php
        ...
        <div class="cm-layout">
            ...
            <div class="cm-settings-col">
                ...
                <div class="cm-tab-pane active" id="cm-pane-kleuren">
                    ...
                    <!-- Thema: Light / Dark sub-tabs -->
                    <div class="cm-group" style="margin-top:8px">
                        <h3 class="cm-group-title" style="display:flex;align-items:center;gap:12px">
                            Thema
                            <span style="font-size:11px;font-weight:400;color:#787c82">Kies het standaard kleurthema van de cookiemelding</span>
                        </h3>
                        <div style="padding:16px 20px 0">
                            <nav class="cm-theme-switcher" style="display:flex;gap:4px;margin-bottom:0;border-bottom:1px solid #c3c4c7;padding-bottom:0">
                                <a class="cm-theme-tab <?php echo ($s['color_theme'] ?? 'light') !== 'dark' ? 'cm-theme-tab-active' : ''; ?>" data-theme-tab="light" href="#" style="padding:8px 16px;font-size:13px;font-weight:500;text-decoration:none;border:1px solid #c3c4c7;border-bottom:none;border-radius:4px 4px 0 0;margin-bottom:-1px;position:relative;z-index:1;background:<?php echo ($s['color_theme'] ?? 'light') !== 'dark' ? '#f6f7f7' : '#fff'; ?>;color:<?php echo ($s['color_theme'] ?? 'light') !== 'dark' ? '#787c82' : '#1d2327'; ?>">
                                    <?php echo ($s['color_theme'] ?? 'light') !== 'dark' ? '✓ ' : ''; ?>☀️ Light
                                </a>
                                <a class="cm-theme-tab <?php echo ($s['color_theme'] ?? 'light') === 'dark' ? 'cm-theme-tab-active' : ''; ?>" data-theme-tab="dark" href="#" style="padding:8px 16px;font-size:13px;font-weight:500;text-decoration:none;border:1px solid #c3c4c7;border-bottom:none;border-radius:4px 4px 0 0;margin-bottom:-1px;position:relative;z-index:1;background:<?php echo ($s['color_theme'] ?? 'light') === 'dark' ? '#f6f7f7' : '#fff'; ?>;color:<?php echo ($s['color_theme'] ?? 'light') === 'dark' ? '#787c82' : '#1d2327'; ?>">
                                    <?php echo ($s['color_theme'] ?? 'light') === 'dark' ? '✓ ' : ''; ?>🌙 Dark
                                </a>
                            </nav>
                            <input type="hidden" name="color_theme" id="cm-color-theme-value" value="<?php echo esc_attr($s['color_theme'] ?? 'light'); ?>">
                        </div>

                        <!-- Light pane -->
                        <div class="cm-theme-pane <?php echo ($s['color_theme'] ?? 'light') !== 'dark' ? '' : 'cm-theme-hidden'; ?>" id="cm-theme-pane-light" style="padding:0 0 8px">
                        <div style="padding:12px 20px 0;display:flex;justify-content:flex-end">
                            <button type="button" class="button cm-reset-theme-defaults" data-theme="light" style="font-size:12px">↺ Standaard kleuren herstellen</button>
                        </div>
                        <?php echo cm_render_theme_fields('light', $s); ?>
                        </div>

                        <!-- Dark pane -->
                        <div class="cm-theme-pane <?php echo ($s['color_theme'] ?? 'light') === 'dark' ? '' : 'cm-theme-hidden'; ?>" id="cm-theme-pane-dark" style="padding:0 0 8px">
                        <div style="padding:12px 20px 0;display:flex;justify-content:flex-end">
                            <button type="button" class="button cm-reset-theme-defaults" data-theme="dark" style="font-size:12px">↺ Standaard kleuren herstellen</button>
                        </div>
                        <?php echo cm_render_theme_fields('dark', $s); ?>
                        </div>
                    </div>
                ...
                <div class="cm-tab-pane" id="cm-pane-layout">
                    ...
                    <!-- Voorkeuren-venster detailniveau -->
                    <div class="cm-group">
                        <h3 class="cm-group-title">Voorkeuren-venster</h3>
                        <div style="padding:16px">
                            <p class="description" style="margin:0 0 16px;padding:0">Kies hoeveel detail bezoekers te zien krijgen in het voorkeuren-venster. De toggle-vinkjes per categorie blijven altijd zichtbaar.</p>
                            <?php $detail = isset($s['prefs_cookie_detail']) ? (int)$s['prefs_cookie_detail'] : 1; ?>
                            <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px;max-width:560px">

                                <!-- Optie 1: Gedetailleerd -->
                                <label style="display:block;border:2px solid <?php echo $detail === 1 ? '#2271b1' : '#dcdcde'; ?>;border-radius:6px;padding:12px;cursor:pointer;transition:border-color .15s;background:<?php echo $detail === 1 ? '#f0f6fb' : '#fff'; ?>">
                                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
                                        <input type="radio" name="prefs_cookie_detail" value="1" <?php checked($detail, 1); ?> style="margin:0">
                                        <span style="font-size:13px;font-weight:600;color:#1d2327">Gedetailleerd</span>
                                    </div>
                                    <div style="background:#f0f0f1;border-radius:4px;padding:8px 10px">
                                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
                                            <span style="font-size:10px;font-weight:600;color:#1d2327">Analytisch</span>
                                            <span style="font-size:9px;color:#787c82">toggle</span>
                                        </div>
                                        <div style="font-size:9px;color:#555;margin-bottom:4px">Beschrijving van de categorie...</div>
                                        <div style="background:#fff;border-radius:3px;padding:4px 6px;margin-bottom:2px">
                                            <div style="font-size:9px;font-weight:600;color:#1d2327">_ga</div>
                                            <div style="font-size:8px;color:#787c82">Google Analytics &middot; 2 jaar</div>
                                        </div>
                                        <div style="background:#fff;border-radius:3px;padding:4px 6px">
                                            <div style="font-size:9px;font-weight:600;color:#1d2327">_gid</div>
                                            <div style="font-size:8px;color:#787c82">Google Analytics &middot; 1 dag</div>
                                        </div>
                                    </div>
                                    <div style="font-size:11px;color:#787c82;margin-top:6px">Toont alle cookies per categorie (huidig)</div>
                                </label>

                                <!-- Optie 2: Vereenvoudigd -->
                                <label style="display:block;border:2px solid <?php echo $detail === 0 ? '#2271b1' : '#dcdcde'; ?>;border-radius:6px;padding:12px;cursor:pointer;transition:border-color .15s;background:<?php echo $detail === 0 ? '#f0f6fb' : '#fff'; ?>">
                                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
                                        <input type="radio" name="prefs_cookie_detail" value="0" <?php checked($detail, 0); ?> style="margin:0">
                                        <span style="font-size:13px;font-weight:600;color:#1d2327">Vereenvoudigd</span>
                                    </div>
                                    <div style="background:#f0f0f1;border-radius:4px;padding:8px 10px">
                                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
                                            <span style="font-size:10px;font-weight:600;color:#1d2327">Analytisch</span>
                                        ...
                                        <div style="font-size:9px;color:#555">Beschrijving van de categorie...</div>
                                    </div>
                                    <div style="font-size:11px;color:#787c82;margin-top:6px">Alleen categorienaam en beschrijving, geen cookielijst</div>
                                </label>
                    ...
                    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">
                        ...
                        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 14px;background:#fff5f5;border:1px solid #f5c6c7;border-radius:4px;cursor:pointer">
                            ...
                            <span>
                                ...
                                <span style="display:block;font-size:12px;color:#646970;margin-top:2px">Verwijdert de licentiegegevens. De banner werkt niet meer totdat u opnieuw een licentie activeert.</span>
                            </span>
                        </label>
                    </div>
