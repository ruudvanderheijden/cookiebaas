...
(function($){
    ...
    function applyPreview() {
        var popupBg       = getColor('color_popup_bg')         || '#ffffff';
        var titleClr      = getColor('color_title')            || '#111111';
        var bodyClr       = getColor('color_body')             || '#444444';
        var acceptBg      = getColor('color_accept_bg')        || '#111111';
        var acceptTxt     = getColor('color_accept_text')      || '#ffffff';
        var acceptHoverBg = getColor('color_accept_hover_bg')  || '#0091ff';
        var acceptHoverTxt= getColor('color_accept_hover_text')|| '#ffffff';
        var rejectBg      = getColor('color_reject_bg')        || '#111111';
        var rejectTxt     = getColor('color_reject_text')      || '#ffffff';
        var rejectBdr     = getColor('color_reject_border')    || 'transparent';
        var acceptBdr     = getColor('color_accept_border')    || 'transparent';
        var prefsBdr      = getColor('color_prefs_border')     || '#d5d0c8';
        var prefsTxt      = getColor('color_prefs_text')       || '#555555';
        var allowBg       = getColor('color_allowall_bg')      || '#111111';
        var allowTxt      = getColor('color_allowall_text')    || '#ffffff';
        var allowHoverBg  = getColor('color_allowall_hover_bg')|| '#0091ff';
        var allowBdr      = getColor('color_allowall_border')  || 'transparent';
        var closeBg       = getColor('color_close_bg')         || '#f0ede8';
        var closeHoverBg  = getColor('color_close_hover_bg')   || '#111111';
        var closeIcon     = getColor('color_close_icon')       || '#555555';
        var toggleOn      = getColor('color_toggle_on')        || '#0091ff';
        var alwaysBg      = getColor('color_always_bg')        || '#e8f4ff';
        var catBorder     = getColor('color_cat_border')       || '#e8e4de';
        var popupR        = parseInt($('#radius_popup').val()); if (isNaN(popupR)) popupR = 20;
        var btnR          = parseInt($('#radius_btn').val());   if (isNaN(btnR))   btnR = 10;
        var overlayOp     = parseInt($('[name="overlay_opacity"]').val()); if (isNaN(overlayOp)) overlayOp = 62;
        var rejectHoverBg  = getColor('color_reject_hover_bg')  || '#e8e4de';
        var rejectHoverTxt = getColor('color_reject_hover_text') || '#111111';
        ...
        var vars = {
            ...
            '--cm-expand-bg': getColor('color_expand_bg') || '#f0ede8',
            '--cm-expand-icon': getColor('color_expand_icon') || '#666666',
            '--cm-expand-open-bg': getColor('color_expand_open_bg') || '#111111',
            '--cm-expand-open-icon': getColor('color_expand_open_icon') || '#ffffff',
            '--cm-cookie-item-bg': getColor('color_cookie_item_bg') || '#ffffff',
            '--cm-service-bg': getColor('color_service_bg') || '#f6f4f1'
        };
        ...
        $('<style id="cm-hover-style">' +
            ...
            '.cm-prev-viewport .cm-allow-all:hover{background:' + allowHoverBg + '!important}' +
            '.cm-prev-viewport .cm-prefs-close:hover{background:' + closeHoverBg + '!important;color:#fff!important}' +
        '</style>').appendTo('head');
    ...
    $(document).on('click', '#cm-reset-all', function() {
        if (!confirm('LET OP: Dit reset ALLES — instellingen, cookielijst, privacyverklaring, consent log én consent data. Dit kan niet ongedaan worden gemaakt. Weet u het zeker?')) return;
    ...
    $(document).on('change', '#cm-dm-toggle', function() {
        $('#cm-dm-fields').toggle(this.checked);
    });
